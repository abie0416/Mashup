...
//     , mediaKeysObserver(0)
 ...
 #endif
 {
     ...
     if (videoWidget != 0) {
         connect(hideWindowButton, SIGNAL(clicked(bool)), videoWidget, SLOT(setHideWindow(bool)));
         connect(videoWidget, SIGNAL(hideWindowChanged(bool)),
                 hideWindowButton, SLOT(setChecked(bool)));
     } else {
...
//     mediaKeysObserver = new MediaKeysObserver(this);
     ...
     connect(controls, SIGNAL(hideWindow()), this, SIGNAL(hideWindow()));
...
//     connect(mediaKeysObserver, SIGNAL(mediaKeyPressed(MediaKeysObserver::MediaKeys)), this, SLOT(handleMediaKeyEvent(MediaKeysObserver::MediaKeys)));
     ...
     controlLayout->addStretch(1);
     controlLayout->addWidget(hideWindowButton);
...
//     handleHideWindow(qobject_cast<QMainWindow *>(this->parent())->ishideWindow());
 }
...
// }
 ...
 {
     return player->metaData(QtMultimediaKit::AlbumArtist).toString();
 }

 QString Player::getSongName()
 {
     return player->metaData(QtMultimediaKit::Title).toString();
 }
